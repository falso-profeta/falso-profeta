    ...
    let 
        do xs acc prev = 
            case xs of
                ...
                x::ys -> 
                    let 
                        next = List.head ys |> Maybe.map (\h -> h.name) |> Maybe.withDefault "final"
                        entry = { story = x, next = next, prev = prev}
                    in
...
getStory k cfg = Maybe.map (\x -> x.story) <| getEntry k cfg
...
getNextStory k cfg = getEntry k cfg |> Maybe.map (\x -> x.next)


getPrevStory : String -> Model -> Maybe String
getPrevStory k cfg = getEntry k cfg |> Maybe.map (\x -> x.prev)
