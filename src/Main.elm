...
pageFromRoute r cfg = 
    case r of
        Intro -> IntroPage
        Error url -> ErrorPage url
        ViewStory key ->
            case Cfg.getStory key cfg of
                Just story -> StoryPage (StoryPage.init story cfg)
                Nothing -> ErrorPage key
        Overview -> OverviewPage (OverviewPage.init cfg)


init : Cfg.Model -> Model
init cfg = { page = IntroPage, cfg = cfg }


view : Model -> Html Msg
view m = case m.page of 
    IntroPage -> IntroPage.view (OnPushUrl (Cfg.firstStory m.cfg |> Maybe.withDefault "final"))
    ErrorPage st ->  div [] [ text ("error :" ++ st)]
    StoryPage m_ ->  Html.map OnStoryMsg (StoryPage.view m_) 
    OverviewPage m_ ->  Html.map OnOverviewMsg (OverviewPage.view m_) 
...
update msg m = 
    ...
    case (msg, m.page) of
        ...
        (OnFetchStories (Ok sts), _) ->
            ({ m | cfg = m.cfg |> Cfg.setStories sts }, Cmd.none)
