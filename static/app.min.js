...
{
	if (region.K.y === region.S.y)
	{
		return 'on line ' + region.K.y;
	}
	return 'on lines ' + region.K.y + ' through ' + region.S.y;
}
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{aC: false, aE: r.aE, aI: r.aI, W: r.W, aN: r.aN, aX: r.aX, au: r.au, a$: r.a$}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{aE: $elm$http$Http$emptyBody, aI: r.aI, W: _List_Nil, aN: 'GET', aX: $elm$core$Maybe$Nothing, au: $elm$core$Maybe$Nothing, a$: r.a$});
};
var $author$project$Model$IntroPage = {$: 0};
var $author$project$Model$Model = F3(
	function (stories, state, transition) {
		return {L: state, z: stories, aZ: transition};
	});
var $author$project$Model$Reset = 3;
var $author$project$Model$ShowCover = 0;
var $author$project$Model$defaultStory = {O: '<default>', Q: 'Bolsonaro, Deputado Federal', U: _List_Nil, E: '', G: '<default>', ai: _List_Nil, aj: '<default>', L: 0, av: '<default>', aB: ''};
							...
							return function (image) {
								return function (events) {
									return function (rants) {
										return {O: bible, Q: context, U: events, E: image, G: name, ai: rants, aj: ref, L: state, av: utter, aB: youtube};
									};
								};
							};
						};
...
var $author$project$Model$Event = F4(
	function (title, text, source, image) {
		return {E: image, aq: source, at: text, aY: title};
	});
	...
	switch (_v0.$) {
		case 0:
			return _Utils_update(
				m,
				{
					L: $author$project$Model$ShowStory,
		...
		case 1:
			...
			if (!_v1.a.$) {
				...
				return _Utils_update(
					m,
					{
						L: $author$project$Model$FinishPage(false)
					});
			}
		default:
			var bool = _v0.a;
			return _Utils_update(
				m,
				{
					L: $author$project$Model$IntroPage,
...
var $author$project$Update$toggleState = F3(
	function (a, b, m) {
		var state = _Utils_eq(m.L, a) ? b : (_Utils_eq(m.L, b) ? a : m.L);
		return _Utils_update(
			m,
			{L: state});
	});
var $author$project$Update$toggleStoryState = F3(
	function (a, b, st) {
		return _Utils_eq(st.L, b) ? _Utils_update(
			st,
			{L: a}) : _Utils_update(
			st,
			{L: b});
...
var $author$project$Main$mainWrapper = function (view) {
	var getJsonRequest = $elm$http$Http$get(
		{
			aI: A2($elm$http$Http$expectJson, $author$project$Update$FetchStories, $author$project$Model$modelDecoder),
			a$: './static/data.json'
		});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2($author$project$Ui$youtubeIframe, '', st.aB),
						A2(
						$elm$html$Html$div,
						_List_Nil,
						_List_fromArray(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$target('_blank'),
					$elm$html$Html$Attributes$alt(source.G)
				]),
			_List_fromArray(
				[elem]));
	};
...
var $author$project$View$viewShowMore = function (st) {
	...
	var fabMiddle = (st.L === 1) ? A2($author$project$Ui$fab, $author$project$Update$ToggleRants, 'fas fa-minus') : A2($author$project$Ui$fabText, $author$project$Update$ToggleRants, 'saiba mais');
	var backCover = A2(
		$elm$html$Html$a,
		_List_fromArray(
			[
	...
	var navChildren = $elm$core$List$isEmpty(st.ai) ? _List_fromArray(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_Utils_ap(
					A2($elm$core$List$map, $author$project$View$viewRant, st.ai),
					_List_fromArray(
						[
							A2(
							$elm$html$Html$div,
...
var $author$project$View$viewStory = function (st) {
	return A4(
		...
		'.' + st.E,
		true,
		A2($author$project$Model$Quote, st.O, st.aj),
		(st.L === 3) ? $author$project$View$viewEvents(st) : ((st.L === 2) ? $author$project$View$showVideoOverlay(st) : $author$project$View$viewShowMore(st)));
};
