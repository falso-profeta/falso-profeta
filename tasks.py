...
def build(ctx, compress=False):
    ...
    if compress:
        ...
        ctx.run("uglifyjs static/app.js --compress 'pure_funcs=[F2,F3,F4,F5,F6,F7,F8,F9,A2,A3,A4,A5,A6,A7,A8,A9],pure_getters,keep_fargs=false,unsafe_comps,unsafe' | uglifyjs --mangle --output static/app.js")
...
def dev(ctx, hot_reload=False):
    ...
    if hot_reload:
        flags.append('-H')
    flags = ' '.join(flags)
    ctx.run(f"elm-live src/Main.elm {flags} -vu -s main.html -- --output static/main.js")
